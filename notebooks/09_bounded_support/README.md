...
Set `FORECAST_PROJECT_ROOT` to the experiment workspace and `TSLIB_ROOT` to a Time-Series-Library checkout containing `models/PatchTST.py`. Dataset discovery follows the conventions in the repository-level `DATA_LAYOUT.md`.
...
Notebook outputs and execution counters are intentionally cleared in this public distribution to avoid embedding machine-specific traces while leaving all executable cells and frozen result artifacts intact.
